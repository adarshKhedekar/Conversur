...
const DocMsg = ({ el }: any) => {
    ...
    return (
        <div className={`flex ${el.incoming ? 'justify-start ' : 'justify-end'}  w-[300px] break-all`}>
            ...
            <span className={`flex flex-col p-2 w-full ${el.incoming ? 'text-textColor bg-background' : 'bg-gradient-to-br from-primaryGradientFrom to-primaryGradientTo/[0.7] text-white'} rounded-lg`}>
...
const ImgMsg = ({ el }: any) => {
    ...
    return (
        <div className={`flex ${el.incoming ? 'justify-start' : 'justify-end'} rounded-lg`}>
            <span className={`flex flex-col p-2 ${el.incoming ? 'text-textColor bg-background' : ' bg-gradient-to-br from-primaryGradientFrom to-primaryGradientTo/[0.7] text-white'} rounded-lg max-w-[300px] break-all`}>
...
const NormalMsg = ({ el }: any) => {
    ...
    return (
        <div className={`flex ${el.incoming ? 'justify-start' : 'justify-end'} rounded-lg`}>
            <p className={`flex py-2 px-4 ${el.incoming ? 'text-textColor bg-background' : 'bg-gradient-to-br from-primaryGradientFrom to-primaryGradientTo/[0.7] text-white'} rounded-lg text-sm max-w-[50%] break-all`}>
